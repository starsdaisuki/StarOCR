...
| 类型 | 扩展名 | API 路径 | 分段处理 |
|------|--------|---------|---------|
| PDF | `.pdf` | 上传 → 签名 URL → `document_url` | 是（>=40MB 自动拆） |
...
`ocr_large_pdf()` 用 PyMuPDF 把 PDF 拆成多个 chunk_size 页的临时文件，逐个走上传路径，最后合并 Markdown。**DOCX/PPTX 无法分段**，若超过 40MB 只能给用户警告。

**为什么阈值是 40MB 而不是 Mistral 文档说的 50MB？** 2026-04 实测一个 46.8MB 的 PDF 触发了 `httpx.RemoteProtocolError: Server disconnected without sending a response` —— multipart/form-data 编码会让请求体比原文件大 1-3%，加上 Mistral 生产环境的实际上限可能比文档严格，所以留 20% 安全余量。
...
PyMuPDF 只能处理 PDF。对于超大的 DOCX/PPTX（>40MB），只能给出警告，上传可能失败。如果未来要支持，可以考虑用 `python-docx` / `python-pptx` 拆分，但不建议——这些文件很少超过 40MB，YAGNI。

### Mistral 上传上限实测比文档严格

文档说"50MB 上传限制"，但 2026-04 实测一个 46.8MB 的 PDF 在 `client.files.upload()` 阶段直接被服务端 `RemoteProtocolError: Server disconnected`。怀疑原因：

1. multipart/form-data 编码后请求体比文件大 1-3%
2. Mistral 生产环境实际限制比文档严格

**修复**：把分段阈值从 50MB 降到 40MB（在 `process_one()` 里）。新阈值留了 20% 安全余量，应对 multipart 开销 + Mistral 实际边界。同时图片的"base64 过大警告"阈值也对应从 40MB 降到 30MB。
...
- Mistral API 文档说 50MB 上传限制，但实测要按 40MB 算（见上方"Mistral 上传上限实测比文档严格"），大 PDF 分段逻辑不可删除
- 新增支持的扩展名时记得同步更新：
